...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        ...
        if random.random() < 0.5:  # 50%の確率で弾を撃つ敵
            enemy_bullet = Enemy_Bullet(self.rect.centerx, self.rect.bottom, self.speed)
...
def main():
    ...
    while running:
        ...
        hit_damage = pygame.sprite.spritecollide(player.sprite, enemy_bullets, True)
        if hit_damage:
            damage += len(hit_damage)
            if damage >= 10:
                print(f"Game Over! Final Score: {score}")
                running = False
        ...
        # スコア表示
        score_text = font.render(f"Score: {score} Damage: {damage}", True, WHITE)
